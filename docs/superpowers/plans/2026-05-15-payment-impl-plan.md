...
- Modify: `src/main/kotlin/.../JavaSpringbootJooqSampleApplication`(이름은 kotlin) — `@EnableScheduling` 추가
...
- [ ] java-jooq: `src/main/resources/db/migration` Flyway migration에 추가. 테스트 전용 `db/mysql/payment-schema.sql`은 만들지 않음
...
- [ ] java-jooq: `src/main/resources/db/migration` Flyway migration에 outbox DDL 추가. 테스트 전용 `db/mysql/outbox-schema.sql`은 만들지 않음
...
- [ ] 양쪽 스키마 원본 갱신: kotlin은 JPA entity, java-jooq는 Flyway migration만 수정
- [ ] java-jooq: 테스트 전용 `db/mysql/compensation-schema.sql`, `db/mysql/cancellation-schema.sql`은 만들지 않음
- [ ] java-jooq seed/reset 데이터는 `src/test/resources/db/common`만 사용하고 DDL은 Flyway에만 둠
