...
> `OrderUseCase.payOrder`는 트랜잭션 없이 결제 흐름을 조율하고, `OrderPaymentTransactionService`가 `preparePayOrder`, `markPaymentApproved`, `markPaymentFailed`, `completePayOrder`의 짧은 DB 트랜잭션만 담당한다. 이렇게 해야 PG approve 같은 외부 호출 동안 DB 커넥션/락을 점유하지 않으면서도, 시나리오 A/B에서 Payment audit과 주문/Outbox 저장 경계를 분리해 매트릭스대로 검증할 수 있다.
>
...
| 호출 | propagation | 비고 |
|---|---|---|
| `OrderUseCase.payOrder` | NOT_SUPPORTED | 결제 오케스트레이션. PG.approve를 어떤 DB 트랜잭션에도 묶지 않음 |
| `OrderPaymentTransactionService.preparePayOrder` | REQUIRED | 주문 잠금 조회 + Payment.REQUESTED 저장 후 commit |
| `OrderPaymentTransactionService.markFailed` | REQUIRES_NEW | 시나리오 A에서 Payment.FAILED audit 별도 commit |
| `OrderPaymentTransactionService.markApproved` | REQUIRES_NEW | PG.approve 성공 직후 Payment.APPROVED audit 별도 commit |
| `OrderPaymentTransactionService.completePayOrder` | REQUIRED | Order PAID 전이 + OutboxEvent.save만 하나의 짧은 트랜잭션으로 보호 |
| `OrderUseCase.cancelOrder` | NOT_SUPPORTED | 취소 오케스트레이션. PG.refund를 어떤 DB 트랜잭션에도 묶지 않음 |
| `OrderPaymentTransactionService.prepareCancelOrder` | REQUIRED | cancellation insert + Order CANCELLED 전이 후 commit |
| `OrderPaymentTransactionService.recordCancellationRefundOutcome` | REQUIRED | 환불 결과에 따라 cancellation.status 기록 |
| `CompensationService.compensateApprovedPayment` | NOT_SUPPORTED | PG.refund 외부 호출. 결과 기록은 transaction service에 위임 |
| `CompensationTransactionService.recordRefundSucceeded/recordRefundFailedAndSchedule` | REQUIRES_NEW | Payment 상태 전이 + 실패 시 CompensationTask insert |
| `CompensationTransactionService.claimPendingTasks` | REQUIRES_NEW | SKIP LOCKED로 task를 짧게 claim하고 lease 시각으로 갱신 |
| `CompensationService.processCompensationTask` | NOT_SUPPORTED | Worker 재시도 오케스트레이션. PG.refund 재시도는 트랜잭션 밖에서 수행 |
